"""Strictly validate ST_* serial evidence and derive v0.2 tables from it.

The parser never runs SensorTrust itself. It uses the exact flags emitted by
the real C core on the board and keeps sample and episode statistics separate.
"""
...
SAMPLE_COLUMNS = (
    ...
    "phase", "raw_value", "raw_valid", "injection", "fault_active",
    "expected_fault", "injected_value", "injected_valid", "detected_flags",
    "active_flags", "health_score", "state",
)
EPISODE_COLUMNS = (
    ...
    "recovery_latency_ms", "recovery_latency_samples", "observed_any_fault",
)
METRIC_COLUMNS = (
...
def parse_log(text: str, config: dict, config_hash: str,
    ...
    for index, (sample, expected) in enumerate(zip(samples, schedule)):
        phase, run, mode, active, target = expected
...
def episodes(samples: list[dict], config: dict) -> list[dict]:
    ...
    for mode in PLAN:
        ...
        for run in range(1, config["repetitions"] + 1):
            ...
            first = next((row for row in active if row["expected_fault"] == target and
            ...
            rows.append({
                ...
                "observed_any_fault": any(row["detected_flags"] for row in active),
            })
...
def metrics(samples: list[dict], episode_rows: list[dict]) -> list[dict]:
    ...
    for fault, bit in BITS.items():
        ...
        for sample in samples:
            truth = sample["expected_fault"]
            if sample["fault_active"] and truth not in (fault, "NONE"):
                continue  # another injection can legitimately cause extra flags
            is_true = truth == fault
...
def clean_baseline(samples: list[dict], config: dict) -> dict:
    ...
    false_counts = {name: sum(bool(row["detected_flags"] & bit) and
                              row["expected_fault"] != name for row in clean)
                    for name, bit in BITS.items()}
    flagged = sum(any(row["detected_flags"] & bit and row["expected_fault"] != name
...
def evaluate(log_path: Path, out_dir: Path, config_path: Path = CONFIG,
    ...
    sample_rows = [{
        ...
        **{key: row[key] for key in SAMPLE_COLUMNS if key not in
           ("experiment_id", "run", "sample_index", "timestamp_ms", "active_flags")},
        "active_flags": None,
    } for row in samples]
